...
function StickerTileImage({ imageKey, index }: { imageKey: string; index: number }) {
  const source = imageKey
    ? `/api/preview-sticker?key=${encodeURIComponent(imageKey)}&index=${index}`
    : "/sticker-sheet.png";
  return <div className="sticker-image"><img className="sticker-slice-source" src={source} alt="" /></div>;
}
