...
export default function Home(){
  ...
  const [paymentPlan,setPaymentPlan]=useState<"one-time"|"subscription">("one-time");
  ...
  useEffect(()=>{const params=new URLSearchParams(window.location.search);const checkout=params.get("checkout");const sessionId=params.get("session_id");if(checkout==="cancelled"){queueMicrotask(()=>setCheckoutNotice("Checkout was cancelled. Your preview is still here if you want to try again."));window.history.replaceState({},"",window.location.pathname);return}if(!sessionId){if(checkout)window.history.replaceState({},"",window.location.pathname);return}
    ...
    const poll=async()=>{
      try{
        const raw=await fetch(`/api/checkout-status?session_id=${encodeURIComponent(sessionId)}`).then(r=>r.json()) as {paid?:boolean;pending?:boolean;email?:string|null;downloadUrl?:string};
        if(!raw.paid){
        ...
        startTransition(()=>{setStage("confirmation");setCheckoutSessionId(sessionId);if(raw.email)setEmail(raw.email);if(raw.downloadUrl)setGeneratedImage(raw.downloadUrl)});
        window.history.replaceState({},"",window.location.pathname);
      }catch{
