...
import { ChangeEvent, DragEvent, RefObject, startTransition, useEffect, useLayoutEffect, useRef, useState } from "react";
import { ArrowLeft, ArrowRight, Check, ChevronDown, Download, ImagePlus, PawPrint, Sparkles, Trash2, Upload } from "lucide-react";
...
function UploadBox({ previews, pet=false, home=false, inputRef, onChange, onRemove }: { previews:string[]; pet?:boolean; home?:boolean; inputRef?:RefObject<HTMLInputElement|null>; onChange:(f:FileList|null)=>void; onRemove:(index:number)=>void }) {
  ...
  const dropLabel=previews.length?`✓ ${previews.length} PHOTO${previews.length>1?"S":""} ADDED`:home?"Drop a photo here":"DROP YOUR PHOTOS HERE";
  const uploadLabel=previews.length===0?home?"Upload a photo":"UPLOAD PHOTOS":previews.length<MAX_PHOTOS?"ADD ANOTHER PHOTO":"3 PHOTO LIMIT";
  return <label className="upload-zone" onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add("dragging")}} onDragLeave={e=>e.currentTarget.classList.remove("dragging")} onDrop={drop}><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp,image/heic,image/heif,.heic,.heif" multiple disabled={previews.length>=MAX_PHOTOS} onChange={(e:ChangeEvent<HTMLInputElement>)=>{onChange(e.target.files);e.currentTarget.value=""}}/><div className="upload-icon">{pet?<PawPrint/>:<ImagePlus/>}</div><b>{dropLabel}</b><span><Upload size={15}/>{uploadLabel}</span>{previews.length>0&&<div className="preview-row">{previews.map((src,index)=><div className="preview-item" key={src}><img src={src} alt={`Upload ${index+1}`}/><button type="button" aria-label={`Delete photo ${index+1}`} onClick={event=>{event.preventDefault();event.stopPropagation();onRemove(index)}}><Trash2/></button></div>)}</div>}</label>;
}
...
export default function Home(){
  ...
  return <main className={`shell ${stage}${isV2Home?" landing-v2":""}`}><div className="grain"/>{stage!=="confirmation"&&<nav><button className="logo" onClick={restart}>SALTY STICKER<sup>™</sup></button><span aria-hidden="true"/><div className="nav-end">{signedIn ? (
    ...
    if(stage==="home"){track("header_upload_click");if(heroFileInputRef.current){heroFileInputRef.current.click();return}}
    if(stage==="samples"){setSkipPhotoStep(false);setStage("photos");return}
    restart()
  ...
  </div><footer className="wizard-legal"><a href="/privacy" target="_blank" rel="noopener noreferrer">Privacy</a><a href="/terms" target="_blank" rel="noopener noreferrer">Terms</a><a href="/refunds" target="_blank" rel="noopener noreferrer">Refunds</a><a href="mailto:hello@saltysticker.com">Support</a></footer></section>}
  ...
  <Dialog open={!isActiveMember&&paymentOpen} onOpenChange={setPaymentOpen}><DialogContent className="payment-modal">
    <><DialogHeader><DialogTitle>How do you want your stickers?</DialogTitle></DialogHeader><div className="purchase-options"><button className={paymentPlan==="digital"?"selected":""} onClick={()=>setPaymentPlan("digital")}><b>DIGITAL — $4.99</b><span>10 transparent PNG stickers<br/>Full sticker-sheet PNG<br/><strong>Instant download</strong></span></button><button className={paymentPlan==="physical"?"selected":""} onClick={()=>setPaymentPlan("physical")}><b>PHYSICAL + DIGITAL — $9.99</b><em>★ MOST POPULAR</em><span>10 waterproof die-cut stickers<br/>Digital pack included<br/><strong>Free shipping</strong></span></button><button type="button" className={`membership-upsell ${paymentPlan==="membership"?"selected":""}`} onClick={()=>setPaymentPlan("membership")}><p className="membership-title">♡ MAKE IT MONTHLY</p><p className="membership-head">Join Salty Sticker Club · $14.99/month</p><p className="membership-free">Today’s sticker order is included when you join.</p><p><strong>Create up to 20 sticker sheets every month</strong></p><p><strong>Choose 3 favorites to get printed &amp; delivered</strong></p><p><strong>Download every sticker sheet you create</strong></p><p className="membership-cancel"><strong>Free shipping · Cancel anytime</strong></p></button></div>{checkoutError&&<p role="alert">{checkoutError}</p>}<Button className="black-btn pay-card" disabled={paymentPlan==="membership"?(subscriptionLoading||!canVerify):(checkoutLoading||!canVerify)} onClick={()=>paymentPlan==="membership"?void startSubscription():void startCheckout()}>{paymentPlan==="membership"?(subscriptionLoading?"OPENING…":"JOIN SALTY STICKER CLUB — $14.99"):(checkoutLoading?"OPENING STRIPE…":paymentPlan==="physical"?"GET MY STICKERS — $9.99":"GET DIGITAL STICKERS — $4.99")} <ArrowRight/></Button></>
