...
function StickerTileImage({ source, index }: { source: string; index: number }) {
  const column = index === 9 ? 1 : index % 3;
  const row = Math.floor(index / 3);
  return <div className="sticker-image"><img className="sticker-slice-source" src={source} alt="" style={{ left: `-${column * 100}%`, top: `-${row * 100}%` }} /></div>;
}
