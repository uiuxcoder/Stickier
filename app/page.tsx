...
import { ArrowLeft, ArrowRight, Check, ChevronDown, Download, ImagePlus, Package, PawPrint, Sparkles, Trash2, Upload } from "lucide-react";
...
function PreviewWatermark({ className = "" }: { className?: string }) {
  return <div className={`preview-watermark ${className}`} aria-hidden="true">{Array.from({length:9},(_,index)=><span key={index}>SALTY STICKER · PREVIEW</span>)}</div>;
}
function previewImageKey(source: string) {
  try {
    return new URL(source, "https://preview.local").searchParams.get("key") || "";
  } catch {
    return "";
  }
}
function StickerTileImage({ imageKey, index }: { imageKey: string; index: number }) {
  const source = imageKey ? `/api/preview-sticker?key=${encodeURIComponent(imageKey)}&index=${index}` : null;
  return <div className="sticker-image">{source ? <img key={source} className="sticker-slice-source" src={source} alt="" /> : null}</div>;
}
...
export default function Home(){
  ...
  {stage==="reveal"&&<section className="reveal-page enter"><div className="reveal-head"><div><h2>{isActiveMember?"We've added them to your creations!":<>Your sticker<br/>sheet <span className="reveal-ready-line">is ready!</span></>}</h2>{isActiveMember?<p>These are included in your subscription.</p>:null}{generationError&&<p role="alert">{generationError}</p>}{checkoutError&&<p role="alert">{checkoutError}</p>}</div><div className="reveal-side">{isActiveMember?<div className="member-reveal-saved"><a className="member-download-primary" href={`/api/download-stickers?image_key=${encodeURIComponent(generatedImageKey)}`} download><Download/> DOWNLOAD STICKERS</a><a className="member-portal-secondary" href="/account">RETURN TO PORTAL <ArrowRight/></a></div>:<><div className="reveal-actions"><Button className="red-btn" onClick={openPayment} disabled={Boolean(generationError)}>GET MY STICKERS <ArrowRight/></Button></div>{paymentOpen&&turnstileSiteKey&&!generationError&&generatedImageKey?<div ref={mountTurnstile} className="turnstile-widget reveal-turnstile"/>:null}</>}</div></div><div className="reveal-body"><div className="sticker-grid">{positions.map((_,i)=><div className={`sticker-tile ${i===9?"sticker-tile-last":""}`} key={i}><span>{String(i+1).padStart(2,"0")}</span><StickerTileImage imageKey={generatedImageKey||previewImageKey(generatedImage)} index={i}/><PreviewWatermark/></div>)}</div><aside className="full-sheet-preview"><article className="reveal-sheet-card"><header><span>SALTY STICKER™</span><b>01 / 01</b></header><img src={generatedImage||"/sticker-sheet.png"} alt={`${subject}'s full sticker sheet`} onError={event=>{event.currentTarget.src="/sticker-sheet.png"}}/><PreviewWatermark className="sheet-preview-watermark"/><footer>ONE OF ONE · MADE FOR YOU</footer></article></aside></div></section>}
